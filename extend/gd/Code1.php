...
final class BCG_Label
{
    ...
    public function setPosition($position)
    {
        ...
        if ($position !== self::POSITION_TOP && $position !== self::POSITION_RIGHT && $position !== self::POSITION_BOTTOM && $position !== self::POSITION_LEFT) {
            error('The text position must be one of a valid constant.');
        }
    ...
    public function setAlignment($alignment)
    {
        ...
        if ($alignment !== self::ALIGN_LEFT && $alignment !== self::ALIGN_TOP && $alignment !== self::ALIGN_CENTER && $alignment !== self::ALIGN_RIGHT && $alignment !== self::ALIGN_BOTTOM) {
            error('The text alignment must be one of a valid constant.');
        }
    ...
    public function draw($im, $color, $x, $y)
    {
        if ($this->rotationAngle !== 0) {
            if (!function_exists('imagerotate')) {
                error('The method imagerotate doesn\'t exist on your server. Do not use any rotation.');
            }
...
abstract class BCG_Barcode
{
    ...
    public function setScale($scale)
    {
        $scale = intval($scale);
        if ($scale <= 0) {
            error('The scale must be larger than 0.');
        }
    ...
    public function setOffsetX($offsetX)
    {
        ...
        if ($offsetX < 0) {
            error('The offset X must be 0 or larger.');
        }
    ...
    public function setOffsetY($offsetY)
    {
        ...
        if ($offsetY < 0) {
            error('The offset Y must be 0 or larger.');
        }
...
abstract class BCG_Barcode1D extends BCG_Barcode
{
    ...
    public function setThickness($thickness)
    {
        ...
        if ($this->thickness <= 0) {
            error('The thickness must be larger than 0.');
        }
    }
...
final class BCG_code128 extends BCG_Barcode1D
{
    ...
    public function setStart($table)
    {
        if ($table !== 'A' && $table !== 'B' && $table !== 'C' && $table !== NULL) {
            error('The starting table must be A, B, C or NULL.');
        }
                ...
                if ($text[$pos + 1] === '~') {
                    ...
                    if (isset($text[$pos + 2])) {
                        ...
                        if ($v >= 1 && $v <= 4) {
                            ...
                            error('Bad ~F. You must provide a number from 1 to 4.');
                        }
                    } else {
                        error('Bad ~F. You must provide a number from 1 to 4.');
                    }
                } else {
                    error('Wrong code after the ~.');
                }
            } else {
                error('Wrong code after the ~.');
            }
        } else {
            error('There is no ~ at this location.');
        }
    }
        ...
        if ($this->tilde) {
            ...
            while (($pos = strpos($text, '~', $previousPos)) !== false) {
                ...
                if ($simpleTilde && $currentMode !== 'B') {
                    error('The Table ' . $currentMode . ' doesn\'t contain the character ~.');
                }

                // At this point, we know we have ~Fx
                if ($tildeData !== '~F1' && $currentMode === 'C') {
                    // The mode C doesn't support ~F2, ~F3, ~F4
                    error('The Table C doesn\'t contain the function ' . $tildeData . '.');
                }

                $length = $pos - $previousPos;
                if ($currentMode === 'C') {
                    if ($length % 2 === 1) {
                        error('The text "' . $text . '" must have an even number of character to be encoded in Table C.');
                    }
                }
    ...
    private function setParseA($text, &$currentMode)
    {
        ...
        if (preg_match('/[^' . $tmp . ']/', $text, $match) === 1) {
            ...
            error('The text "' . $text . '" can\'t be parsed with the Table A. The character "' . $match[0] . '" is not allowed.');
        } else {
    ...
    private function setParseB($text, &$currentMode)
    {
        ...
        if (preg_match('/[^' . $tmp . ']/', $text, $match) === 1) {
            // We found something not allowed
            error('The text "' . $text . '" can\'t be parsed with the Table B. The character "' . $match[0] . '" is not allowed.');
        } else {
    ...
    private function setParseC($text, &$currentMode)
    {
        ...
        if (preg_match('/[^' . $tmp . ']/', $text, $match) === 1) {
            // We found something not allowed
            error('The text "' . $text . '" can\'t be parsed with the Table C. The character "' . $match[0] . '" is not allowed.');
        } else {
    ...
    private function getSequence(&$text, &$starting_text)
    {
        ...
        for ($x = 0; $x < $xLen; $x++) {
            ...
            if ($c === 0) {
                ...
                error('Character ' . $input . ' not supported.');
            }
