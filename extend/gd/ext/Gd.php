...
class Gd
{
    ...
    public static function draw($im, array $option)
    {
        ...
        if (!_CLI and ($option['save'] === 0 or $option['save'] === 2)) {
...
//                header('Expires: ' . gmdate('D, d M Y H:i:s \G\M\T', time() - 1), true);
     ...
     */
    public static function createIM(string $pic, int $type = 0)
    {
        if (is_bool($type)) {
     ...
     * @return null|resource
     */
    public static function createFromImg(string $filename)
    {
        //打开文件，若出错则退
