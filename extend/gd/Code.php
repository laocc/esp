...
class Code
{
    private static $option = [
        ...
        'cn_font' => [_ROOT . 'font/ygyxsziti2.0.ttf' => 0, _ROOT . 'font/simkai.ttf' => 1],
//        'en_font' => [_ROOT . 'font/ariblk.ttf' => 0, _ROOT . 'font/arial.ttf' => 1],
        'en_font' => [_ROOT . 'font/FZCYJW.ttf' => 0, _ROOT . 'font/hyb7gjm.ttf' => 1, _ROOT . 'font/fzkan.ttf' => 1],
    ...
    public static function create(array $option = [])
    {
        ...
        if (!isset($option['code']) or !$option['code']) {//没指定code，则保存到cookies中
            ...
            $addContent = strtoupper($option['cookies']['attach'] . implode($code));//附加串，有效期最长1小时
            $enCode = password_hash($addContent, PASSWORD_DEFAULT);
            setcookie($option['cookies']['key'], $enCode, 0, '/', '.' . _HOST, _HTTPS, true);//输出之前先保存Cookies
        }
        $option = [
    ...
    public static function check($input)
    {
        ...
        $ck = self::$option['cookies'];
        $ck['attach'] .= date($ck['date']);
        $cookies = isset($_COOKIE[$ck['key']]) ? $_COOKIE[$ck['key']] : null;
        $addContent = strtoupper($ck['attach'] . $input);
        setcookie($ck['key'], null, -1, '/', '.' . _HOST, _HTTPS, true);
        return (!!$cookies and !!$input and password_verify($addContent, $cookies)) ? true : false;
    }
    ...
    private static function createCode1(&$img, &$code, $opt)
    {
        ...
        if (!isset($opt['code']) or !$opt['code']) {
            ...
            $code = array_rand(array_flip(str_split(self::$disc[$cn ? 'cn' : 'en'], $cn ? 3 : 1)), $_code_len);
        } else {
