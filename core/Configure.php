...
final class Configure
{
     ...
     * @return array
     */
    public function forceRedis(): array
    {
     ...
     * @return mixed
     */
    public function hash(string $fn): mixed
    {
     ...
     * @return bool
     */
    public function select(int $dbID = -1): bool
    {
...
//        if ($key === ['*']) return $this->_CONFIG_;
     ...
     */
    private function asyncRPC(bool $json): mixed
    {
        /**
    ...
    private function connectRedis(array $conf, int $db): Redis
    {
        ...
        try {
            ...
            if ($tryCont++ > 2) {
                ...
                esp_error('Configure', $error->getMessage(), $err);
            }
            usleep(1000);
            goto tryCont;
        }
...
//        if (_CLI) return false;//cli都不读缓存
     ...
     * @param array $conf
     */
    private function load_redis(array $conf): void
    {
     ...
     * @return void
     */
    private function loadHash(array $conf): void
    {
     ...
     * @return void
     */
    private function load_file(array $conf): void
    {
    ...
    private function mergeConfig(array $conf): void
    {
        $config = [];
