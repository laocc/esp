...
//        return (new Builder($this, $this->_CONF['prefix'], 0))->table($tabName);
        return (new Builder($this, $this->_CONF['prefix'], $this->transID))->table($tabName);
    }
    ...
    private function connect(bool $upData, int $trans_id = 0)
    {
        ...
        $cnf = $this->_CONF;
        if (!$upData) {
            $host = isset($cnf['slave']) ? $cnf['slave'] : $cnf['master'];
        ...
        try {
            $opts = array(
                ...
                \PDO::ATTR_TIMEOUT => (isset($cnf['timeout']) and $cnf['timeout'] > 0) ? $cnf['timeout'] : 5, //设置超时时间，秒，默认=2
            );
            if ($cnf['port'] == 0) {//unix_socket
                $conStr = "mysql:dbname={$cnf['db']};unix_socket={$host};charset={$cnf['charset']};id={$trans_id};";
            } else {
                $conStr = "mysql:dbname={$cnf['db']};host={$host};port={$cnf['port']};charset={$cnf['charset']};id={$trans_id};";
            }
            try {
    ...
    {
        if (preg_match('/^(select|insert|replace|update|delete|alter)\s+.+/is', trim($sql), $matches)) {
            return $matches[1];
        } else {
    ...
    public function query_exec(string $sql, array $option, \PDO $CONN = null)
    {
        ...
        if (!in_array($action, ['select', 'insert', 'replace', 'update', 'delete', 'alter'])) {
...
//                    $this->PdoAttribute($CONN);//显示所有属性
        ...
        //数据操作时，若当前trans_run=false，则说明刚才被back过了或已经commit，后面的数据不再执行
        if ($upData and //更新操作
...
//            pre($error);
    ...
    public function trans(int $trans_id = 1, array $batch_SQLs = [])
    {
        ...
        return new Builder($this, $this->_CONF['prefix'], $trans_id);
    }
