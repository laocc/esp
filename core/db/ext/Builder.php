...
final class Builder
{
    ...
    private $bindKV = Array();

    public function __construct(Mysql $mysql, $table_pre, $trans_id = 0)
    {
        $this->clean_builder();
        $this->_MySQL = $mysql;
    ...
    private function clean_builder($clean_all = true)
    {
        ...
        $this->bindKV = $this->_param_data = Array();
    ...
    public function where($field = '', $value = null, $is_OR = null)
    {
        ...
        } elseif (substr($field, -1) === '~') {//组合 like

            if (substr($value, 0, 1) === '^') $value = substr($value, 1);
            else if (substr($value, 0, 1) !== '%') $value = "%{$value}";

            if (substr($value, -1) === '$') $value = substr($value, 0, -1);
            else if (substr($value, -1) !== '%') $value = "{$value}%";

            $_where = substr($field, 0, -1) . ' like ' . $this->quote($value);
...
//            if (preg_match('/\x20*?(<|>|<=|>=|=|<=>|\!=|<>)\x20*?$/', $protectFiled)) {
//                $protectFiled .= ' ';
//
//            } elseif (preg_match('/\x20in$/', $protectFiled)) {
//                $protectFiled .= ' in ';
//
//            } else {
//                $protectFiled .= ' ';
...
//        return $this->_MySQL->query_exec($sql, $this->option($op), $this->_MySQL->master[$this->_Trans_ID]);
    ...
    public function update(array $data, $add_identifier = true)
    {
        ...
        foreach ($data as $key => &$value) {
            $vFH = substr($value, 0, 1);
            $kFH = substr($key, -1);
            if (in_array($vFH, ['+', '-']) and in_array($kFH, ['+', '-'])) {
                throw new \Exception('DB_ERROR: 键值不可同时带有符号');
            }

            //值以+-开头
            if (in_array($vFH, ['+', '-'])) {
                $val = substr($value, 1);
                $value = $this->protect_identifier($key) . " {$vFH} {$val}";
