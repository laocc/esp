...
{
    echo "<div class=\"bar\"><div class=\"barli\" style=\"width:{$percent}%\">&nbsp;</div></div>";
}
...
//ajax调用实时刷新
if ($_GET['act'] == "rt") {
    $arr = array('freeSpace' => $df, 'TotalMemory' => $mt, 'UsedMemory' => $mu, 'FreeMemory' => $mf, 'CachedMemory' => $mc, 'Buffers' => $mb,
        'TotalSwap' => $st, 'swapUsed' => $su, 'swapFree' => $sf, 'loadAvg' => $load, 'uptime' => $uptime,
//        'freetime' => $freetime, 'bjtime' => $bjtime,
        'stime' => $stime, 'memRealPercent' => $memRealPercent, 'memRealUsed' => $memRealUsed, 'memRealFree' => $memRealFree,
        'memPercent' => "{$memPercent}%", 'memCachedPercent' => $memCachedPercent, 'barmemCachedPercent' => "{$memCachedPercent}%",
        'swapPercent' => $swapPercent, 'barmemRealPercent' => "{$memRealPercent}%", 'barswapPercent' => "{$swapPercent}%",
    );
    for ($i = 2; $i < count($strs); $i++) {
        $arr["NetOut{$i}"] = $NetOut[$i];
        $arr["NetInput{$i}"] = $NetInput[$i];
    }

    $jarr = json_encode($arr, 256);
...
<div id="page">
    ...
    <? if ("show" == $sysReShow) { ?>
        <table width="100%" cellpadding="3" cellspacing="0" align="center">
            ...
            <tr>
                <td>内存使用状况<br \><br \><a href="http://www.vpser.net/other/linux-vps-ram.html" target="_blank"><font
                                color=red>内存参数解读</font></a></td>
                <td colspan="5">
                    <?php
                    $tmp = array(
                    ...
                    if ($sysInfo['memCached'] > 0) {
                        ...
                        <div class="bar">
                            <div id="barmemCachedPercent" class="barli_blue">&nbsp;</div>
                        </div>
                        ...
                        %
                        <div class="bar_1">
                            <div id="barmemRealPercent" class="barli_1">&nbsp;</div>
                        </div>
                        <?php
                    }
                    //判断如果SWAP区为0，不显示
                    if ($sysInfo['swapTotal'] > 0) {
                        ...
                        %
                        <div class="bar">
                            <div id="barswapPercent" class="barli_red">&nbsp;</div>
                        </div>
    ...
    <?php if (false !== ($strs = @file("/proc/net/dev"))) : ?>
        <table width="100%" cellpadding="3" cellspacing="0" align="center">
            <tr>
                <th colspan="3">网络使用状况</th>
            </tr>
            <?php for ($i = 2; $i < count($strs); $i++) : ?>
                <?php preg_match_all("/([^\s]+):[\s]{0,}(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/", $strs[$i], $info); ?>
                <tr>
                    <td width="13%"><?php echo $info[1][0] ?> :</td>
                    <td width="43%">已接收 : <font color='#CC0000'><span
                                    id="NetInput<?php echo $i ?>"><?php echo $NetInput[$i] ?></span></font> GB
                    </td>
                    <td width="43%">已发送 : <font color='#CC0000'><span
                                    id="NetOut<?php echo $i ?>"><?php echo $NetOut[$i] ?></span></font> GB
                    </td>
                </tr>
            <?php endfor; ?>
        </table>
...
?></span>
    ...
    <table width="100%" cellpadding="3" cellspacing="0" align="center">
        <tr>
            <th colspan="4" class="th_1">PHP相关参数</th>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="3"><a href='<?php echo $phpSelf; ?>?act=Function' target='_blank' class='static'>请点这里查看详细！</a>
            </td>
        </tr>
        <tr>
    ...
    <table width="100%" cellpadding="3" cellspacing="0" align="center">
        <tr>
            <th colspan="4" class="th_1">组件支持</th>
        </tr>
        <tr>
    ...
    <table width="100%" cellpadding="3" cellspacing="0" align="center">
        <tr>
            <th colspan="4" class="th_1">第三方组件</th>
        </tr>
        <tr>
    ...
    <table width="100%" cellpadding="3" cellspacing="0" align="center">
        <tr>
            <th colspan="4" class="th_2">数据库支持</th>
        </tr>
        <tr>
    ...
    <form action="<?php echo $_SERVER['PHP_SELF'] . "#bottom"; ?>" method="post">
        ...
        <table width="100%" cellpadding="3" cellspacing="0" align="center">
            <tr>
                <th colspan="3" class="th_2">MySQL数据库连接检测</th>
            </tr>
            <tr>
                <td width="15%"></td>
                <td width="60%">
        ...
        <table width="100%" cellpadding="3" cellspacing="0" align="center">
            <tr>
                <th colspan="3" class="th_4">函数检测</th>
            </tr>
            <tr>
                <td width="15%"></td>
                <td width="60%">
        ...
        <table width="100%" cellpadding="3" cellspacing="0" align="center">
            <tr>
                <th colspan="3" class="th_5">邮件发送检测</th>
            </tr>
            <tr>
                <td width="15%"></td>
                <td width="60%">
    ...
    <div id="footer">
        &copy; 2012 <a href="http://lnmp.org" target="_blank">LNMP一键安装包</a><br/>This Prober was based on Yahei
        Prober.<br/>
