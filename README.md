...
# 三、程序说明：
- 程序结构为` 模块 > 控制器 - 模型 > 动作 > 视图 `，数据模型、视图，均由控制器派生出来；本人觉得这种MVC模式应该称之为`Mcmav`模式。
...
#RewriteCond $1 !^(index\.php|images|static|js|css|data|www\.zip|robots\.txt)
RewriteRule ^((rule|safe|read|join|contact).*)$ index.php/$1 [L]
