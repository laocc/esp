...
final class Builite
{
    ...
    public function __construct(\db\Lite &$callback, $table, $trans_id = 0)
    {
        $this->clean_builder();
    ...
    {
        if (empty($field)) {
            error('DB_ERROR: where in 条件不可为空');
        }
    ...
    public function where_like($field, $value, $is_OR = FALSE)
    {
        if (empty($field) || empty($value)) {
            error('DB_ERROR: where like 条件不能为空');
        }
        $protectField = $this->protect_identifier($field);
    ...
    public function where_group_start($is_OR = FALSE)
    {
        ...
        if ($this->_where_group_in) {
            error('DB_ERROR: 当前还处于Where Group之中，请先执行where_group_end');
        }
        if (empty($this->_where)) {
            $this->_where = '(';
        } else {
    ...
    public function where_group_end()
    {
        if ($this->_where_group_in === false) {
            error('DB_ERROR: 当前未处于Where Group之中');
        }
        if (empty($this->_where)) {
            error('DB_ERROR: 当前where条件为空，无法创建where语句');
        } else {
            $this->_where .= ')';
            $this->_where_group_in = false;
        }
    ...
    public function join($table, $_filter = null, $method = 'left', $identifier = true)
    {
        ...
        if (!in_array($method, [null, 'LEFT', 'RIGHT', 'INNER', 'OUTER', 'FULL', 'USING'])) {
            error('DB_ERROR: JOIN模式不存在：' . $method);
        }
        ...
        foreach ($_filters as $re) {
            $_filter_arr[] = preg_replace_callback('/^(.*)(>|<|<>|=|<=>)(.*)/', function ($matches) use ($identifier) {
                if ($matches[1] === $matches[3]) {
                    error('DB_ERROR: JOIN条件两边不能完全相同，如果是不同表相同字段，请用tabName.filed的方式');
                }
                if ($identifier)
    ...
    public function order($field, $method = 'ASC', $addProtect = true)
    {
        ...
        if (!in_array($method, ['ASC', 'DESC', 'RAND'])) {
            error('DB_ERROR: ORDER模式不存在：' . $method);
        }
    ...
    public function delete()
    {
        ...
        if (empty($where)) {//禁止无where时删除数据
            error('DB_ERROR: 禁止无where时删除数据，如果要清空表，请采用：id>0的方式');
        }
    ...
    {
        if (empty($data)) {
            error('DB_ERROR: 无法 insert/replace 空数据');
        }
        $this->_param_data = [];
