...
//        self::$href[] = ['lv' => $num, 'name' => $name, 'title' => $line];
     ...
     * @return string
     */
    public static function html($text, $addNav = true)
    {
        self::$_footnotes = [];
        self::$_definitions = [];
        self::$_holders = [];
        ...
        self::joinHtml($html);
        return (($addNav and !is_wap()) ? self::makeNav() : '') . $html;
    }
     ...
     * @param $text
     */
    private static function replaceHtml(&$text)
    {
    ...
    private static function makeNav()
    {
        if (!self::$href) return '';
...
//        // 单行``注释
        $text = preg_replace_callback("/(^|[^\\\])\<(?<style>[\w\;\-\:\#\.\% ]+?)?\>(?<val>.+?)\<\/\>/", function ($matches) {
            return $matches[1] . self::makeHolder("<span style='{$matches['style']}'>" . htmlspecialchars($matches['val']) . '</span>');
        }, $text);
        ...
        $text = preg_replace_callback("/<(https?:\/\/.+)>/i", function ($matches) {
            return self::makeHolder("<a href=\"{$matches[1]}\">{$matches[1]}</a>");
        }, $text);
        ...
        $text = preg_replace_callback("/\[((?:[^\]]|\\]|\\[)+?)\]\(((?:[^\)]|\\)|\\()+?)\)/", function ($matches) {
            ...
            $url = self::escapeBracket($matches[2]);
            $target = substr($url, 0, 1) == '#' ? ' target="_self"' : '';
            return self::makeHolder("<a href=\"{$url}\" {$target}>{$escaped}</a>");
        }, $text);

        $text = preg_replace_callback("/\[((?:[^\]]|\\]|\\[)+?)\]\[((?:[^\]]|\\]|\\[)+?)\]/", function ($matches) {
            $escaped = self::parseInline(self::escapeBracket($matches[1]), '', false, false);
            $result = isset(self::$_definitions[$matches[2]]) ?
                ("<a href=\"" . self::$_definitions[$matches[2]] . "\">{$escaped}</a>") : $escaped;
...
//            $f = ['0000BB', '007700', 'DD0000'];//蓝，绿，红
//            $f = ['8cc6ff', 'cfffb5', 'ffd2df'];//蓝，绿，红
            $str = str_replace(['#0000BB', '#007700', '#DD0000', '&lt;?php'], ['#8cc6ff', '#cfffb5', '#ffea00', '//C语言代码'], $str);
    ...
    private static function parseTable(array $lines, array $value)
    {
        ...
        foreach ($lines as $key => &$line) {
            ...
            foreach ($columns as $ky => &$column) {
                ...
                if (preg_match("/^(?:({$color})|(?:(\d+);)){1,2}(.*)$/i", $text, $matches)) {
                    $text = $matches[3];
                    $bgcolor = $matches[1];
                    $num = $matches[2];
                }
